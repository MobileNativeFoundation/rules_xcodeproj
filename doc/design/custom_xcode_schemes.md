...
autogenerated scheme behavior is preserved.
...
Note that the `schemes` attribute for the `xcodeproj` declaration references the scheme target,
`foo_scheme`. There is no `targets` attribute as the targets are inferred from the schemes.
...
be covered in a subsequent section of this document.

The `xcodeproj` logic combines the targets from both schemes to create the final set of targets
...
The following expands on the previous example by adding a `launch_action` rule to identify the launch
target.
...
the buildable targets for the `app_scheme` scheme automatically.
...
The recommendation for this design is to proceed with [Option 1: No Smarts](#option-1-no-smarts). If
a target is specified directly in the `targets` attribute for an `xcodeproj` declaration, an
autogenerated scheme will be created.
...
The first pass of this design did not include a separate `schemes` attribute. Targets and scheme
references were combined in the `targets` attribute. The auhtor opted for the separate `schemes`
attribute as it allows the attribute definitions to be more precise and it provides a bit more
clarity on what should be referenced where.

### Should launch actions be inferred from `targets` in `xcode_scheme`?
