...
bazelrcs=(
  --noworkspace_rc
  "--bazelrc=$installer_inputs_dir/pre_xcodeproj.bazelrc"
...
touch "$installer_inputs_dir/pre_xcodeproj.bazelrc"

readonly bazel_env_file="$installer_inputs_dir/bazel_env.sh"
{
  echo "# Generated by rules_xcodeproj. Do not edit."
  echo "envs=("
  for env in "${envs[@]}"; do
    printf '  %q\n' "$env"
  done
...
installer_flags+=(--bazel_env "$bazel_env_file")
...
if [[ -n "$xcode_version" ]]; then
  cat <<EOF >> "$installer_inputs_dir/pre_xcodeproj.bazelrc"
