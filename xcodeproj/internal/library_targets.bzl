...
def _collect_objc_indexstores(target):
    """Gathers outputs with .indexstore extension from the target's transitive ObjcCompile action outputs 
    """
    objc_compile_actions = [action for action in target.actions if a.mnemonic == "ObjcCompile"]
    objc_compile_action_outputs = [action.outputs for action in objc_compile_actions]
    objc_indexstores = [output for output in depset(transitive = objc_compile_action_outputs).to_list() if output.extension == "indexstore"]
...
def process_library_target(
    ...
    objc_indexstores = _collect_objc_indexstores(target)
