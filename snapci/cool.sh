...
ARCHIVE_SHA=$(cat bazel-output-base/execroot/rules_xcodeproj/bazel-out/darwin_arm64-opt/bin/distribution/release.tar.gz.sha256 | cut -d ' ' -f 1)
...
gsutil cp bazel-output-base/execroot/rules_xcodeproj/bazel-out/darwin_arm64-opt/bin/distribution/release.tar.gz "$GCS_URL"
