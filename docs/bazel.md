...
| <a id="xcodeproj-temporary_directory"></a>temporary_directory |  Optional. The directory where rules_xcodeproj will write some temporary files. You will want to add this directory to your source control ignore files (e.g. <code>.gitignore</code>). A Bazel package will be created at this path, so you can use the path in <code>visibility</code>/<code>package_group</code>s. The path is relative to the workspace root. Defaults to <code>.rules_xcodeproj</code>.   |  <code>".rules_xcodeproj"</code> |
