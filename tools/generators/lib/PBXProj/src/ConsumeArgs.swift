...
extension AsyncThrowingStream<String, Error>.Iterator {
    // MARK: - consumeArg
    ...
    public mutating func consumeArg<Output> (
        ...
        guard let rawArg = try await next() else {
            throw PreconditionError(
                message: url.prefixMessage("Missing <\(name)>"),
                file: file,
                line: line
...
"""),
    ...
    public mutating func consumeArgs<Output>(
        ...
        let count = try await consumeArg("\(name)-count", as: Int.self, in: url)
