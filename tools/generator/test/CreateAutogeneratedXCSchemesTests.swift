...
class CreateAutogeneratedXCSchemesTests: XCTestCase {
    ...
    func assertScheme(
        ...
        case .none:
            shouldExpectLaunchAction = false
        ...
        if let testAction = scheme.testAction {
            guard shouldExpectTestAction else {
                XCTFail(
                    "Did not expect a test action for \(scheme.name)",
                    file: file,
                    line: line
                )
                return
        ...
        } else if shouldExpectTestAction {
            XCTFail(
        ...
        if let launchAction = scheme.launchAction {
            guard shouldExpectLaunchAction else {
                XCTFail(
                    "Did not expect a launch action for \(scheme.name)",
                    file: file,
                    line: line
                )
                return
    ...
    """,
        ...
        } else if shouldExpectLaunchAction {
            XCTFail(
...
""",
    ...
    func assertBazelSchemes(
        ...
        // DEBUG BEGIN
        fputs("*** CHUCK schemes.map(\\.name):\n", stderr)
        for (idx, item) in schemes.map(\.name).enumerated() {
