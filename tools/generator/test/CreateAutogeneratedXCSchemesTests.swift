...
class CreateAutogeneratedXCSchemesTests: XCTestCase {
    ...
    func assertScheme(
        ...
        let expectedHostBuildableReference: XCScheme.BuildableReference?
        if let hostTargetKey = hostTargetKey {
            ...
            expectedHostBuildableReference = .init(
                pbxTarget: aTarget,
                referencedContainer: filePathResolver.containerReference
            )
        } else {
            expectedHostBuildableReference = nil
        }

        let expectedBuildConfigurationName = target.defaultBuildConfigurationName
        let expectedPBXTargetInfo = XCScheme.PBXTargetInfo(
            pbxTarget: target,
            referencedContainer: filePathResolver.containerReference
        )
        let expectedBuildableReference = expectedPBXTargetInfo.buildableReference
        // let expectedBuildableReference = XCScheme.BuildableReference(
        //     pbxTarget: target,
        //     referencedContainer: filePathResolver.containerReference
        // )
        ...
        case .set:
            expectedBuildPreActions = [.clearBazelBuildOutputGroupsFile] + [
                .init(targetInfo: expectedPBXTargetInfo, hostIndex: hostIndex)!,
            ]
        case .none:
